...
export function LoginForm() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (authData.user) {
      // Fetch user role from profiles table to ensure correct redirect
      ...
      <div className="relative z-10 w-full max-w-[420px] rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-auth-card)] p-8 shadow-2xl">
        ...
        <form className="mt-6 space-y-5" onSubmit={handleEmailLogin}>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex justify-center items-center gap-2 rounded-xl bg-[var(--accent-violet)] py-3.5 text-sm font-semibold text-[var(--accent-violet-on)] transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <button
          ...
          disabled={isLoading}
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-auth-inner)] py-3 text-sm font-medium text-[var(--text-primary)] transition-colors hover:border-[var(--border-hover)] hover:bg-[var(--social-hover-bg)] disabled:opacity-50"
        >
          <GoogleIcon />
